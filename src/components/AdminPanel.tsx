...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const handleTestConnection = async () => {
    ...
    const currentMapping: ColumnMapping = {
      ...
      total: letterToIdx(mappingStr.total),
  ...
  const fieldsDetail: Array<{ key: keyof ColumnMapping; label: string; placeholder: string; desc: string }> = [
    ...
    { key: "total", label: "ยอดรวมสินค้า+ส่ง (Total Amount)", placeholder: "F", desc: "คอลัมน์รวมยอดทั้งหมด (F)" },
    { key: "paid", label: "ประวัติโอนแล้ว (Paid Amount)", placeholder: "G", desc: "ยอดรวมที่ชำระเข้าบัญชีแล้ว (G)" },
    { key: "balance", label: "ค้างจ่ายคงค้าง (Remaining Balance)", placeholder: "H", desc: "ยอดหนี้ค้างส่งร้านค้า (H)" },
    { key: "status", label: "สถานะสั่งซื้อส่งของ (Order Status)", placeholder: "I", desc: "เช่น จัดส่งสำเร็จ / ถึงไทย / รอกดเว็บ (I)" },
    { key: "orderId", label: "รหัสคำสั่งซื้ออ้างอิง (Order Reference)", placeholder: "H", desc: "เช่น ORD-xxxx" },
    { key: "date", label: "ตัวเลขวันรอบเรือ (Shipping Round Date / Column J)", placeholder: "J", desc: "วันที่สถานะรอบเรือของลูกค้า" },
    { key: "notes", label: "หมายเหตุร้าน (Merchant Note / Column I)", placeholder: "I", desc: "รายละเอียดบันทึกเพิ่มเติมหรือวันที่ลงประวัติ" },
